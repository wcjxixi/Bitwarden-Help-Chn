...
创建 Bitwarden 帐户的任何人都将拥有自己的个人密码库，可以从任何客户端应用程序访问。个人密码库对每个用户都是唯一的，并且只有该用户拥有访问它的钥匙，使用他们的电子邮件地址和主密码的组合。帐户所有者对个人帐户以及存储在其中的个人[密码库项目](../your-vault/vault-items.md)负责。组织[所有者、管理员和经理](../organizations/user-types-and-access-control.md)在设计上无法看到任何其他用户的个人密码库，从而确保某人的个人数据仍然属于他们自己。
...
Bitwarden 使用了一种企业友好的方法来实现大规模的共享。用户可以被添加到具有[多个不同角色](../organizations/user-types-and-access-control.md)的组织中，隶属于不同的[群组](onboarding-and-succession.md#groups)，并将这些群组分配给不同的[集合](onboarding-and-succession.md#collections)，以实现精细化访问权限。可用的角色中有一个[自定义角色](../organizations/user-types-and-access-control.md#custom-role)，它用于管理权限的精细化配置。
...
| **组织中的权限**     | 作为[经理](../organizations/user-types-and-access-control.md)，Alice 可以管理集合的许多方面。 |
...
{% hint style="success" %}
除了这些好处外，客户还很欣赏将 Bitwarden 紧密集成到他们现有系统中的能力。Bitwarden 提供了一个强大的公共 [API](https://bitwarden.com/help/api/) 和一个功能齐全的[命令行界面](../getting-started/bitwarden-cli.md)（CLI），以进一步整合到现有的组织工作流程中。
{% endhint %}
...
**答：**可以！使用我们[全面的基于角色的访问控制套件](../organizations/user-types-and-access-control.md#access-control)，您可以将凭据设为**只读**以防止复制副本。
