# 企业帮助台

{% hint style="success" %}
对应的[官方文档地址](https://bitwarden.com/help/courses/password-manager-enterprise-helpdesk/)
{% endhint %}

作为 IT 支持专业人员，当员工遇到 Bitwarden 问题时，您是他们的首选求助对象。本培训涵盖 Bitwarden 企业版最常见的帮助台场景，从重置主密码到排查组织用户密码库访问问题，应有尽有。分步骤的视频教程将帮助您快速解决员工问题，确保组织访问权限的安全和不间断。

## 访问权限 <a href="#access" id="access"></a>

<details>

<summary>批准设备请求</summary>

学习如何在 Admin Console 中审核和批准设备访问请求，以便员工可以快速重新访问他们的密码库。

{% embed url="https://vimeo.com/1191294901?fl=pl&fe=sh" %}

* **00:00**：简介
* **00:04**：登录网页 App
* **00:10**：Admin Console
* **00:18**：设备批准
* **00:20**：查看请求
* **00:26**：批准请求

</details>

## 恢复 <a href="#recovery" id="recovery"></a>

<details>

<summary>恢复成员账户</summary>

轻松恢复忘记了主密码或丢失所有受信任设备的成员账户。

{% embed url="https://vimeo.com/1190598451?fl=pl&fe=sh" %}

* **00:00**：简介
* **00:08**：登录网页 App
* **00:15**：Admin Console
* **00:20**：定位会员
* **00:29**：恢复账户
* **00:57**：Bitwarden Send
* **01:21**：分享链接

</details>

## 安全 <a href="#security" id="security"></a>

<details>

<summary>发送密码更改请求</summary>

Bitwarden 可以轻松提示用户更改存在风险的密码。

{% embed url="https://vimeo.com/1192034974?fl=pl&fe=sh" %}

* **00:00**：简介
* **00:05**：登录网页 App
* **00:11**：Admin Console
* **00:15**：Access Intelligence
* **00:24**：选择关键 App
* **00:31**：分配任务
* **00:37**：成员视图

</details>

## 支持 <a href="#support" id="support"></a>

<details>

<summary>Smart Search</summary>

使用 **Bitwarden Smart Search** 立即寻找答案。

<a href="https://bitwarden.com/smart-search/" class="button primary">Try Smart Search</a>

</details>

<details>

<summary>询问社区</summary>

与经验丰富的 Bitwarden 用户联系，分享技巧并共同解决问题。在[社区论坛](https://community.bitwarden.com/c/support/pm-ask-the-community/?_gl=1*1gc3ljx*_gcl_au*NDkyNTE5NzM4LjE3NjU1Mzk5OTUuMTMxODM0NjMwLjE3NjY4OTA0MjUuMTc2Njg5MDQyNQ..*_ga*MTk5NzU0OTg5My4xNzU2OTg5NjI5*_ga_QBRN562QQQ*czE3Njc2MzE2MjUkbzE3OCRnMSR0MTc2NzYzMjUxNyRqNjAkbDAkaDA.)或 [Reddit](https://www.reddit.com/r/Bitwarden/new/) 上提问。

</details>

<details>

<summary>联系支持</summary>

遇到技术问题或有账户问题？请联系 Bitwarden 支持团队获取直接的协助。

<a href="https://bitwarden.com/contact/" class="button primary">联系支持</a>

</details>
