...
{% tabs %}
{% tab title="移动端" %}
要登录更多账户，请从顶部菜单栏中选择当前已登录的账户，然后选择 <i class="fa-plus">:plus:</i>**添加账户**。
...
选择 <i class="fa-plus">:plus:</i>**添加账户**将带您进入登录界面：
...
{% hint style="info" %}
大多数密码库操作，包括添加新项目或文件夹、同步、自动填充以及[密码库超时](vault-timeout-options.md)和解锁（[PIN 码](more-log-in-unlock-methods/unlock-with-pin.md) 或[生物识别](more-log-in-unlock-methods/unlocking-with-biometrics.md)）等设置仅适用于活动账户，您可以通过 App 顶部菜单栏中显示的图标来确定活动账户。
...
{% tab title="桌面端" %}
要登录更多账户，请从桌面 App 的右上角选择当前已登录的账户，然后选择 <i class="fa-plus">:plus:</i>**添加账户**。
...
{% hint style="info" %}
大多数密码库操作，包括添加新项目或文件夹、同步、搜索以及[密码库超时](vault-timeout-options.md)和解锁（[PIN 码](more-log-in-unlock-methods/unlock-with-pin.md) 或[生物识别](more-log-in-unlock-methods/unlocking-with-biometrics.md)）等设置仅适用于活动账户，您可以通过显示在 App 右上角中的电子邮件来确定活动账户。
...
选择账户图标后，从账户切换菜单中选择 <i class="fa-plus">:plus:</i>**添加账户**：
...
{% hint style="info" %}
登录多个账户不会在账户之间转移项目。如果您想将保存的项目从一个 Bitwarden 账户移动到另一个账户，请[导入您的数据](../../password-manager/import-and-export/import-data.md)。
{% endhint %}
