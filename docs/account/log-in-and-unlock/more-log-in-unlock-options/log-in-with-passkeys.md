...
<div data-with-frame="true"><figure><img src="https://bitwarden.com/assets/2gsO1o5tDU7s7LXvcpaL7w/56a8a797155760fe6fbd1d2e2d92b59b/2025-12-31_11-19-58.png?w=999&#x26;fm=avif" alt=""><figcaption><p>使用通行密钥用于密码库加密</p></figcaption></figure></div>
...
<div data-with-frame="true"><figure><img src="https://bitwarden.com/assets/TpXTFNlF2hzRaUaLmxAXr/6710e5ab7c98efcb11547dc6038fdf7d/Passkeys_list.png?w=832&#x26;fm=avif" alt=""><figcaption><p>通行密钥列表</p></figcaption></figure></div>

您的通行密钥列表显示了每个通行密钥的加密状态：**已用于加密**、支持但未激活（**设置加密**）或**不支持加密**。
...
### 丢失或删除了通行密钥 <a href="#lost-or-deleted-passkey" id="lost-or-deleted-passkey"></a>

如果您丢失或删除了通行密钥（例如安全密钥、Windows Hello 或 Apple 密码中的通行密钥），则无法再使用该通行密钥访问您的密码库。在这种情况下，您需要使用主密码才能解锁密码库，因此主密码必须足够复杂且易于记忆。建议您创建并妥善保管一个[安全应急包](https://bitwarden.com/resources/bitwarden-security-readiness-kit/)，以便即使丢失了通行密钥，也能访问您的关键的账户信息。
...
1. 通过 WebAuthn API，验证器生成一个**通行密钥公钥和私钥对**。根据定义，该密钥对构成了您的通行密钥。
2. 通过 WebAuthn API PRF 扩展，验证器生成一个 **PRF 对称密钥**。该密钥源自于您的通行密钥的特有的**内部机密**和 Bitwarden 提供的**盐化**。
3. Bitwarden 客户端生成一个 **PRF 公钥和私钥对**。PRF 公钥对您的**账户加密密钥**进行加密，您的客户端可以通过登录和解锁来访问该密钥，然后将生成的 **PRF 加密的账户加密密钥**发送到服务器。
4. 使用 **PRF 对称密钥**对 **PRF 私钥**进行加密（参见第 2 步），然后将得到的 **PRF 加密的私钥**发送到服务器。
5. 您的客户端将数据发送到 Bitwarden 服务器，以用于为您的账户创建一个新的通行密钥凭据记录。如果您的通行密钥凭据被注册为支持密码库加密和解密，此记录将包含：
   * 通行密钥名称
   * 通行密钥公钥
   * PRF 公钥
   * PRF 加密的账户加密密钥
...
1. 使用 WebAuthn API 公钥加密技术，您的身份验证请求将被断言和确认。
2. 您的 **PRF 加密的账户加密密钥**和 **PRF 加密的私钥**将从服务器发送到您的客户端。
3. 使用 Bitwarden 提供的相同**盐化**和您的通行密钥特有的**内部机密**，**PRF 对称密钥**在本地被重新创建。
4. **PRF 对称密钥**用于解密您的 **PRF 加密的私钥**，从而生成您的 **PRF 私钥**。
5. **PRF 私钥**用于解密您的 **PRF 加密的账户加密密钥**，从而得到您的**账户加密密钥**。您的账户加密密钥用于解密您的密码库数据。
