...
* 在 Windows 或 Linux 上，导航至**文件** → **设置**以选择您的会话超时时间和会话超时动作。
...
* **如果您关闭浏览器窗口**，您将从网页 App 中注销，以及您的浏览器扩展将超时。
  ...
  * 要在关闭浏览器窗口后改为使用 PIN 码解锁，请在[设置 PIN 码](more-log-in-unlock-methods/unlock-with-pin.md)时取消选中**浏览器重启时使用主密码锁定**选项。
{% endhint %}
{% endtab %}
...
* **锁定**（默认）\
  密码库锁定时将在设备上保留密码库数据，因此可以离线解锁您的密码库。您只需要输入[主密码](../master-password.md)或 PIN 码，或使用[生物识别](more-log-in-unlock-methods/unlocking-with-biometrics.md)，但不需要使用任何有效的两步登录方式。
* **注销**\
...
如果您使用[受信任设备](../../admin-console/login-with-sso/trusted-devices/about-trusted-devices.md)，则必须启用[生物识别](more-log-in-unlock-methods/unlocking-with-biometrics.md)或 [PIN 码](more-log-in-unlock-methods/unlock-with-pin.md)才能解锁您的密码库。如果未启用生物识别或 PIN 码，会话超时将始终为注销而不是锁定。
