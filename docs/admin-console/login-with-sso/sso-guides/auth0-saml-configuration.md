...
| Token Endpoint Authentication Method | 选择 **Client Secret (Post)**，它将映射到[稍后配置](auth0-saml-configuration.md#identity-provider-configuration)的 **Binding Type** 属性。                                      |
...
输入[已配置的组织标识符](generic-saml.md#step-1-set-an-organization-identifier)，然后选择**登录**。如果您的实施已成功配置，您将被重定向到 Auth0 的登录界面：
