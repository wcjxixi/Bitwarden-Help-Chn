...
Bitwarden 目录连接器 App 通过从一系列源目录服务中提取信息，自动为您的 Bitwarden 组织配置用户、群组和群组关联。已配置的用户将收到加入组织的邀请，随后可完成正常的[入职流程](../user-management.md#onboard-users)。

当用户从源目录中被禁用时，目录连接器可被配置为从 Bitwarden 组织中移除用户。这不会删除他们的 Bitwarden 账户，但他们将失去对组织的所有访问权限。

{% hint style="info" %}
目录连接器功能适用于**团队 版**和**企业版**组织。要使用目录连接器，您必须具有[组织 API 密钥](../../bitwarden-public-api.md#authentication)访问权限，该访问权限只​​能由[组织所有者](../member-roles.md)获取并使用 [Bitwarden Send](../../../password-manager/bitwarden-send/about-send.md) 安全共享。
{% endhint %}
