...
运行 Cron 作业时，我们建议使用一个专用的 Directory Connector 用户身份来执行此操作。创建一个 `bwdc` 用户（如果还没有的话），然后将该用户添加到 `etc/cron.allow` 列表中。这将允许非 root 用户设置和运行 Cron 作业。

要继续操作，您还需要获取组织的 [API 密钥](../../bitwarden-public-api.md#authentication)的 `client_id` 和 `client_secret` 值，组织**所有者**可以从网页密码库导航到组织**设置** → **我的组织**获取它们。
...
2. 向 crontab 添加一行，其中包含：
   * 一个[调度表达式](schedule-a-sync.md#cron-job-scheduling-expressions)，它将确定执行所需命令的时间/重复间隔（例如 `0 0 * * 2` 表示在每周二的午夜运行）。
