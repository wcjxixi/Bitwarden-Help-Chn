...
企业版客户可通过有效的且专属于 Bitwarden 的 DNS TXT 记录为其组织声明域名（例如 `mycompany.com`）的所有权。声明域名后，您的组织将获得对具有匹配的电子邮箱地址的账户的额外控制权限：
...
使用具有与域名匹配的电子邮箱地址（例如 `jdoe@mycompany.com`）入职的组织成员账户（称为[已声明账户](claimed-accounts.md)）还将获得以下好处：
...
<div align="left" data-with-frame="true"><figure><img src="https://bitwarden.com/assets/6WJAs5AXufz8zSiVjEp5aA/8d9f4576f877ce74d6553430801070a9/2025-01-14_09-56-53.png?w=991&#x26;fm=avif" alt=""><figcaption><p>声明域名</p></figcaption></figure></div>
...
**如果发现错误的 TXT 记录**，您的 DNS 更改可能需要更长时间才能生效。**如果发现正确的 TXT 记录但声明仍然失败**，则可能是您的 Bitwarden 服务器配置为使用内部 DNS 服务器，而不是使用公共 DNS 服务器进行更新。
{% endhint %}
...
{% hint style="info" %}
Directory Connector 和 SCIM 无法删除已声明账户，只有管理员和所有者可以通过网页 App Admin Console 执行此操作。
{% endhint %}
