...
#### 提示用户将项目移动到组织 <a href="#prompt-users-to-move-items-to-the-organization" id="prompt-users-to-move-items-to-the-organization"></a>

启用**集中组织所有权**策略的此子选项，将提示当前在「**我的密码库**」中存储有项目的用户[将所有此类项目转移到「我的项目」位置下的组织所有权](../../password-manager/organization-members/transfer-ownership.md)。此转移流程只需单击一下即可快速完成。
...
使用此策略的组织成员将无法[使用通行密钥登录](../../account/log-in-and-unlock/more-log-in-unlock-methods/log-in-with-passkeys.md)。
...
启用**禁用 PIN 码解锁**策略后，成员将无法在网页 App、浏览器扩展和桌面 App 上配置或[使用 PIN 码解锁](../../account/log-in-and-unlock/more-log-in-unlock-methods/unlock-with-pin.md)。该策略打开后适用于所有组织成员，包括管理员和所有者。
...
启用**默认 URI 匹配检测**策略以为您的成员设置[默认 URI 匹配检测](../../password-manager/autofill/troubleshoot-autofill/forming-uris-for-autofill.md#default-match-detection)。这可以帮助您配置[自动填充](../../password-manager/autofill/autofill-from/autofill-from-browser-extensions.md)，以最好地满足您组织的安全和策略需求。
