...
事件日志可以从 [Bitwarden 公共 API](../../bitwarden-public-api.md) 的 `/events` 端点访问和导出，并无限期保留，但每次只能查看 367 天的数据（由范围选择器决定）。
...
{% hint style="info" %}
每一个**事件**都使用类型代码（`1000`、`1001` 等）关联，以标识事件所捕获的操作。[Bitwarden 公共 API](../../bitwarden-public-api.md) 使用类型代码来标识被事件所记录的操作。
{% endhint %}
