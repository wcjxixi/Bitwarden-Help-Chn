...
在从 Bitwarden 连接之前，请生成 Bitwarden 用于向 Blumira 发送事件的凭据：
...
4、您的**凭据**将显示出来，其中包括 **HTTP Event Collector URL** 和 **HTTP Event Collector Token**。完成下一章节需要这两个信息。

{% hint style="info" %}
**HTTP Event Collector Token** 只会显示一次。您可以立即将此值复制并保存到安全的位置，或者在完成 Bitwarden 设置的同时，将此窗口保持在单独的浏览器标签页中打开。
{% endhint %}
