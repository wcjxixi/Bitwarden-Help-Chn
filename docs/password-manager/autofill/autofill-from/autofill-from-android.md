...
Bitwarden 可以自动填充您的密码，以便您可以无缝登录网站和 App，同时维护强大而安全的密码。自动填充功能通过检测与您登录的服务相匹配的密码库项目，以减少了您在登录过程中的复制和粘贴操作。
...
* **弹窗**：此选项将在输入字段的弹出窗口中建议自动填充的凭据。

5、如果您使用 Brave 或 Chrome 作为网页浏览器，请打开**使用 Brave 自动填充集成**或**使用 Chrome 自动填充集成**选项，以确保自动填充功能在这些浏览器中能正常工作。更多信息，请参阅[下文](autofill-from-android.md#browser-integrations)。

{% hint style="info" %}
由于 Brave 和 Chrome Android App 的运行机制，除非手动启用浏览器标志，否则自动填充功能在使用 HTTP 的网站上将无法正常工作。要在 Brave/Chrome 上为 HTTP 网站启用自动填充功能：

1. 导航至 `brave://flags`
2. 搜索**被视为安全的不安全来源** ，然后启用该标志
3. 添加所需的 HTTP 域名/IP 地址，例如：`http://192.0.2.0/24` 或 `http://myserver.local`
4. 重启 Brave 浏览器
{% endhint %}
...
{% hint style="info" %}
遇到问题了吗？请参阅我们的 [Android 自动填充故障排除](../troubleshoot-autofill/troubleshooting-android-auto-fill.md)指南。如果您仍然无法使用 Android 自动填充功能，请[联系我们](https://bitwarden.com/contact/)。
{% endhint %}
...
如果您使用 Brave、Chrome 或 Vivaldi 作为您的网页浏览器，请打开**使用 Brave \<browser> 集成**选项（其中 **\<browser>** 是浏览器的名称），以确保自动填充功能在这些浏览器中能正常工作。此操作将引导您进入相应浏览器的设置界面，您需在此启用此选项以使用第三方服务。
...
## 自动填充通行密钥 <a href="#autofill-passkeys" id="autofill-passkeys"></a>

Bitwarden Android App 可用于存储和自动填充通行密钥。要启用自动填充通行密钥，请导航至**设置** → **自动填充**，然后点击**通行密钥管理**以访问 Android 设置，将 Bitwarden 配置为您的通行密钥提供程序。请注意，Android 系统不允许 Bitwarden 等第三方通行密钥提供程序支持基于通行密钥的 2FA（又称「不可发现凭据」）；Bitwarden 存储的通行密钥只能用作主要登录凭据。了解更多有关[在 Android 上存储和自动填充通行密钥](../more-autofill-options/autofill-passkeys.md#android)的信息 。

## 自动填充期间切换账户 <a href="#switch-accounts-during-autofill" id="switch-accounts-during-autofill"></a>

如果您[登录了多个账户](../../../account/log-in-and-unlock/account-switching.md)，您的移动 App 将默认尝试自动填充当前活动账户的凭据。在自动填充过程中，您可以通过轻按头像气泡来切换账户。
