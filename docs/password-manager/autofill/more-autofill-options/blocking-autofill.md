...
被指定为屏蔽的域名将屏蔽自动填充、屏蔽通行密钥提示，以及屏蔽保存或更新凭据提示。
{% endhint %}

要为浏览器扩展指定要屏蔽的域名：

1. 在 Bitwarden 浏览器扩展中，打开 <i class="fa-gear-complex">:gear-complex:</i>**设置**选项卡。
...
{% tab title="Android" %}
{% hint style="info" %}
屏蔽自动填充 URI 目前仅适用于 **Android 8.0 (Oreo)** 或更高版本。
{% endhint %}

要为 Android 指定要屏蔽自动填充的 URI：

1、在 Bitwarden Android App 中，点击 <i class="fa-gear-complex">:gear-complex:</i>**设置**。

2、点击**自动填充**。

3、向下滚动并点击**屏蔽自动填充**。

4、点击**新增屏蔽的 URI**，然后输入 URI。多个 URI 之间使用逗号分隔，例如：
...
**对于 Android App**，[URI 方案](../troubleshoot-autofill/forming-uris-for-autofill.md#uri-schemes)始终以 `androidapp://` 作为开头，这与典型的网络浏览器 URI 有些不一样。例如：
...
对于 iOS 用户，可以使用自动填充功能打开 Bitwarden 获取 App URI。打开 Bitwarden 后，选择屏幕右上角的 <i class="fa-plus">:plus:</i> 图标。从这里复制新的密码库项目中包含的 URI。将此 URI 粘贴到此 App 的现有登录项目中。
{% endhint %}
{% endtab %}
{% endtabs %}
