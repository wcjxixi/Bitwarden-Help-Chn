...
您可以将 Bitwarden Send 与命令行界面 (CLI) 命令结合使用。由于 Send 是 Bitwarden Password Manager 中的一项功能，因此 [Password Manager CLI 文档](../developer-tools/cli/password-manager-cli.md)中的许多命令、选项和概念与这里的都是相关的。
...
3. 使用 `encode` 命令（参阅[详情](../developer-tools/cli/password-manager-cli.md#encode)）对处理后的 JSON 进行编码。
...
如果你在另一个 Bitwarden 应用程序中创建了一个 Send，而这个会话仍然处于活动状态，使用 `bw sync` 命令将提取最近的 Send。有关更多信息，请参阅我们的 [CLI 文档](../developer-tools/cli/password-manager-cli.md)。
