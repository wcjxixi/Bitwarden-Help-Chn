...
**答：**&#x901A;常，这是因为某个 App 的密码库数据已经滞后于网页 App 或其他客户端 App 的数据。执行密码库同步应该可以使所有数据为最新。有关更多信息，请参阅[同步您的密码库](../your-vault/syncing-your-vault.md)。
...
**答：**&#x60A8;可以使用[加密导出](../import-and-export/encrypted-exports.md)（将使用您的[账户加密密钥](../../security/encryption/encryption-key-rotation.md)进行加密）对您的密码库数据进行安全的长期备份。这些备份使用账户加密密钥、组织加密密钥或您选择的密码进行加密。
...
**答：**&#x65E0;法直接从 Bitwarden 打印，但是您可以[将密码库数据导出](../import-and-export/export-vault-data.md)为 `.csv` 或 `.json` 文件然后从文本编辑器中打印。
