...
* **密码库超时**：
  * **超时时间**：设置 Bitwarden 在[超时](../../account/log-in-and-unlock/vault-timeout-options.md#vault-timeout)前保持活动的时长。
  * **超时动作**：设置 Bitwarden 在超时时将执行的动作。
* **管理设备**：使用另一台设备或受信任的设备批准登录请求。
* **其他选项**：
  * **指纹短语**：查看您账户的指纹短语。
  * **两步登录**：打开网页 App 以设置两步登录。
  * **更改主密码**：打开网页 App 以更改您的主密码。
...
* **自动填充建议**：
  * **在表单字段显示自动填充建议**：激活内嵌自动填充菜单。
    * **将身份显示为建议**：在内嵌自动填充菜单中包含身份。
    * **将支付卡显示为建议**：在内嵌自动填充菜单中包含支付卡。
    * **选择图标时显示建议**：点击内嵌图标以激活自动填充菜单，而不仅仅是聚焦于填充字段。
* **将 Bitwarden 设为默认密码管理器**：（仅限 Chrome）禁用浏览器的内置密码管理器，并将 Bitwarden 设为默认。
* **在密码库视图中始终将支付卡显示为自动填充建议**：在密码库视图的自动填充建议区域显示所有支付卡。
* **在密码库视图中始终将身份显示为自动填充建议**：在密码库视图的自动填充建议区域显示所有身份。
* **自动填充快捷键**：
  * **管理快捷键**：为 Bitwarden 设置自定义键盘快捷键。
* **页面加载时自动填充**：
  * **页面加载时自动填充**：激活页面加载时的自动填充。
  * **登录项目的默认自动填充设置**：设置项目在页面加载时自动填充的默认值。此默认值可以被单个项目覆盖。
* **附加选项**：
  * **显示上下文菜单选项**：允许通过上下文菜单访问 Bitwarden 选项（通常使用右键点击）。
  * **自动复制 TOTP**：凭据被用于自动填充后，自动将 TOTP 复制到剪贴板。
  * **清除剪贴板**：设置何时清除从 Bitwarden 复制到您剪贴板的内容。
  * **默认 URL 匹配检测**：设置 Bitwarden 识别自动填充建议的默认方式。
  * **屏蔽域名**：设置一个域名列表，阻止 Bitwarden 为其使用自动填充。
...
* **保存到密码库选项**：
  * **询问保存和使用通行密钥**：网站或服务支持通行密钥时，允许 Bitwarden 创建和自动填充通行密钥。
  * **询问添加登录**：当您使用未存储在 Bitwarden 中的凭据登录时，允许 Bitwarden 提示您新增项目。
  * **询问更新现有登录**：当使用的凭据与 Bitwarden 中存储的凭据不匹配时，允许 Bitwarden 提示您更新项目。
...
* **文件夹**：添加或编辑文件夹。
* **导入项目**：将数据导入您的密码库或导入组织。
* **导出密码库**：从您的密码库或从您的组织中导出数据。
* **回收站**：查看最近删除的项目。
* **立即同步**：手动触发密码库同步。
...
* **密码库自定义**
  * **显示网站图标并获取更改密码的 URL**：允许 Bitwarden 获取存储在您密码库中的凭据的网站图标和更改密码的 URL。
  * **在密码库上显示快速复制操作**：在密码库视图中的每个项目上显示用于复制用户名或密码的按钮。
  * **点击自动填充建议中的项目以填充**：点击密码库视图中的项目以自动填充其凭据，而不是点击填充按钮。
...
* **验证器同步**：
  * **允许验证器同步**：允许 Bitwarden 密码管理器将认证码同步到 Bitwarden 认证器。
* **会话超时**：
  * **超时时间**：设置 Bitwarden 在超时前保持活动的时长。
  * **超时动作**：设置 Bitwarden 在超时时将执行的动作。
* **其他**：
  * **账户指纹短语**：查看您的账户指纹短语。
  * **两步登录**：打开网页 App 以设置两步登录。
...
* **自动填充服务**：允许 Bitwarden 使用您保存的登录信息登录您的设备上的 App 和网站。这将要求您在后续窗口中选择 Bitwarden 作为您首选的服务。
  * **显示自动填充建议**：选择是否在键盘（内嵌）或输入字段上方（弹窗）显示自动填充建议。
* **通行密钥管理**：打开 Android App 设置，将 Bitwarden 设为您的首选的通行密钥提供程序。
  * **特权 App**：了解更多有关 Bitwarden 在您使用通行密钥时如何保护您免受网络钓鱼的信息。
* **使用无障碍功能**：允许在无障碍菜单中将自动填充作为快速操作磁贴使用。这将要求您在后续窗口中为 Bitwarden 开启无障碍功能。
* **自动复制 TOTP**：凭据被用于自动填充后，自动将 TOTP 复制到剪贴板。
* **询问添加项目**：在您使用未保存在您的密码库中的凭据登录时，允许 Bitwarden 提示您保存项目。
* **默认 URI 匹配检测**：设置 Bitwarden 识别自动填充建议的默认方式。
* **阻止自动填充**：设置一个域名列表，阻止 Bitwarden 为其使用自动填充。
...
* **自动填充**：
  * **密码自动填充**：了解如何设置自动填充，然后点击**继续**进入 iOS 菜单，您将在那里进行设置。
...
* **文件夹**：添加或编辑文件夹。
* **导出密码库**：从您的密码库或从您的组织中导出数据。
* **导入项目**：打开网页 App 以将数据导入您的密码库或导入组织。
...
* **显示网站图标**：允许 Bitwarden 获取存储在您的密码库中的凭据的网站图标。
...
* **允许浏览器集成**：(非 Safari) 允许 Bitwarden 浏览器扩展与桌面 App 集成以进行生物识别解锁。在 Safari 中，无需激活此选项即可使用集成功能。
  * **要求浏览器集成验证**：在 Bitwarden 桌面 App 和浏览器扩展之间建立集成时，要求确认指纹短语。
* **使用硬件加速**：如果您遇到图形或性能问题，请关闭此选项。
* **启用 SSH 代理**：允许 Bitwarden 桌面 App 充当 SSH 代理。
* **主题**：切换 Bitwarden 的颜色模式（浅色模式、深色模式或跟随系统）。
...
* **允许 DuckDuckGo 浏览器集成**：(仅限 macOS) 允许 Bitwarden 桌面 App 与 DuckDuckGo macOS 浏览器之间的集成。
* **主题**：切换 Bitwarden 的颜色模式（浅色模式、深色模式或跟随系统）。
* **语言**：选择 Bitwarden 桌面 App 使用的语言。
