...
{% tabs %}
{% tab title="浏览器扩展" %}
在 <i class="fa-gear-complex">:gear-complex:</i>**设置**选项卡中，选择**密码库选项**然后使用**立即同步密码库**按钮。
{% endtab %}

{% tab title="移动端" %}
打开 <i class="fa-gear-complex">:gear-complex:</i>**设置**标签，点击**其他**选项然后点击**立即同步密**。

切换**启用刷新时同步**选项，以允许在 <i class="fa-lock-keyhole">:lock-keyhole:</i>**我的密码库**选项卡上使用下拉手势来同步密码库。
{% endtab %}
...
{% hint style="info" %}
只有[当前活动的账户](../../account/log-in-and-unlock/account-switching.md)会同步，即使密码库被锁定，账户也能被同步。
{% endhint %}
{% endtab %}
...
在某些情况下，VPN 或广告拦截器浏览器扩展可能会干扰 Bitwarden app 与服务器之间的连接。此问题通常出现在浏览器扩展上。请尝试关闭 VPN 或广告拦截器浏览器扩展，然后[手动同步您的密码库](syncing-your-vault.md#manual-sync)。
