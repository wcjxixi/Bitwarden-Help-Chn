...
自定义字段值的字符限制是根据**加密后的字符数量**来计算的。例如，一个 3383 个字符的 RSA-4096 私有 SSH 密钥在加密并存储在您的密码库中时将增长到约 4400 个字符。
{% endhint %}
...
3、在对应的密码库项目的**自定义字段**部分，选择适当的字段类型，然后选择 <i class="fa-plus">:plus:</i>**新增自定义字段**按钮：
