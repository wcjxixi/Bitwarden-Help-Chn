...
{% hint style="info" %}
标有 <i class="fa-star">:star:</i>**星形图标**的是渐进式推出的功能。
{% endhint %}
...
* <i class="fa-star">:star:</i> **使用 SCIM 和 Directory Connector 预配置用户**：通过 SCIM 或 Directory Connector 配置的用户现在可以被配置为[预配置状态](admin-console/manage-members/user-management.md#member-statuses)，而无需立即发出邀请。此功能通过 [SCIM 配置视图](admin-console/manage-members/scim/about-scim.md#invite-users-after-provisioning)中的新设置或 [Directory Connector 同步选项](admin-console/manage-members/directory-connector/sync-options-and-filters.md#automatically-send-email-invitations)进行控制。

{% hint style="success" icon="lightbulb" %}
对于**新组织**以及**尚未使用 SCIM 或 Directory Connector** 的组织，这两个工具默认会被设置为将用户配置为预配置状态，而不是自动发出邀请。

对于**已在使用 SCIM 或 Directory Connector** 的组织，这两个工具将继续自动发出邀请，以免中断现有工作流程。您可以根据需要随时更改该设置。
{% endhint %}

* <i class="fa-star">:star:</i> **填充辅助策略和自定义规则集**：现在可以通过[策略](admin-console/oversight-visibility/enterprise-policies.md#activate-fill-assist)在成员客户端上默认开启填充辅助。同一策略还可用于将 Bitwarden 精选的规则集替换为[由您的组织构建和维护的自定义规则集](admin-console/oversight-visibility/custom-fill-assist-rules.md)。
...
* <i class="fa-star">:star:</i>**「查看登录」上的自动填充按钮**：在浏览器扩展中，从「[查看登录](password-manager/autofill/autofill-from/autofill-from-browser-extensions.md#view-login)」直接自动填充凭据。
