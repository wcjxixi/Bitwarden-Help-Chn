...
{% hint style="info" %}
安全提示：为修复 Bitwarden CLI 本地存储 API 密钥的相关漏洞，本版本之后的下一个 Bitwarden 服务器版本将自动轮换 Bitwarden CLI 用户的个人 API 密钥。如果您在任何自动化工作流中使用了 Bitwarden CLI，请在该版本发布后立即使用新的 API 密钥更新这些工作流，以确保服务连续性。
{% endhint %}
...
* **★默认点击即可自动填充**：在浏览器扩展程序中，您现在只需点击**自动填充建议**中的任意项目，即可插入凭据。此更新移除了填充按钮以及**点击自动填充建议中的项目以填充**设置，使界面更加简洁。
* **更新了“清除剪贴板”的默认设置**：浏览器扩展程序的清除剪贴板设置现默认设为五分钟，这是一项额外的预防措施，有助于保障您的凭据安全。
* i**OS 现已支持 mTLS**：iOS 和 Android 移动 App 现均支持为需要客户端身份验证的自托管服务器上传 mTLS 证书。在设置服务器 URL 时上传您的证书。
* **附件状态加载条**：在桌面 App 和浏览器扩展中上传附件时，将显示上传状态条。
* **Keeper JSON 导入器**：您现在可以通过 JSON 文件从 Keeper 导入数据。
