...
* **★Blumira SIEM 集成**：团队版和企业版组织现在可以与 Blumira 集成 ，以实现安全和事件管理 (SIEM)。
* **扩展的账户恢复选项**：账户恢复功能现已支持已撤销的成员，并可重置两步登录方式。
* **成员被撤销原因**：在**成员** → **已撤销**页面上，每位成员旁边新增了一个工具提示，说明该成员为什么从您的组织中被撤销。
* **更新了事件日志**：当附件被添加到组织拥有的项目时，Password Manager 现在会使用现有的为项目创建附件事件进行记录。

### 自托管 <a href="#self-host" id="self-host"></a>

* **Helm Chart v2.0**：自托管 Helm Chart 的 2.0 版本将包含两项重大变更：
  * 此版本的图表**默认禁用** NGINX `ingress`。图表支持使用 Gateway API 作为替代方案。
  * `values.yaml` 文件中已移除 `image.name` 字段。使用 `image.name` 的所有配置都必须更新为使用 `image.repository`。

{% hint style="info" %}
在升级到此版本的图表之前，请审查这些更改并相应地更新您的值文件。更多有关设置或迁移到 Gateway API 的信息，请参阅 Helm 流量路由文档 。
{% endhint %}
