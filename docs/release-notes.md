...
* **重命名将项目添加到文件夹**：在密码库视图中，将项目添加到文件夹的选项已从「移动所选」重命名为「添加到文件夹」。[了解更多](password-manager/your-vault/vault-items/folders.md#move-items-to-a-folder)。
...
* **浏览器扩展 - 从保存栏中选择文件夹**：您现在可以直接从浏览器扩展的保存提示中选择要将项目保存到哪个[文件夹](password-manager/your-vault/vault-items/folders.md)（参阅[这里](password-manager/getting-started/getting-started-browserext.md#add-a-login)）。
