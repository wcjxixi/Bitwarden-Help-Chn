...
* **查看登录上的自动填充按钮**：直接从浏览器扩展中的查看登录页面自动填充凭据。
* **Flatpak 生物识别解锁**：Flatpak 安装的桌面 App 现在支持生物识别解锁。
* **增加最小 KDF 迭代次数**：如果您的 PBKDF2 KDF 迭代次数低于 600,000（自 2023.2.0 版本以来的默认级别），您将被要求更新此设置，或者在您下次使用主密码登录或解锁时自动应用此增加的值。

### 管理控制台 <a href="#admin-console" id="admin-console"></a>

* **Huntress SIEM 集成**：Bitwarden 团队版和企业版组织现在可以与 Huntress 集成以用于安全信息和事件管理 (SIEM)。

### Secrets Manager

* **Jenkins 集成**：Bitwarden Secrets Manager CLI 现在可用于将机密注入 Jenkins Pipelines。
...
* **API - 新的成员管理端点**：[公共 API](https://bitwarden.com/help/api/) 现在包括用于[撤销和恢复](admin-console/manage-members/revoke-remove/temporarily-revoke-access.md)组织成员的访问权限的端点。
