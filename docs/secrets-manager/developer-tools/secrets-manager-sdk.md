...
Secrets Manager 软件开发工具包 (SDK) 可帮助开发人员创建用于机密管理的应用程序。Bitwarden 团队使用 Secrets Manager SDK 来构建与 [GitHub Actions](https://bitwarden.com/help/github-actions-integration/) 等流行产品的离散集成，社区成员可以使用它来构建自己的应用程序。

核心 SDK（可在[这里](https://github.com/bitwarden/sdk)找到）使用 Rust 编写，并提供 Rust API、CLI 和 Node-API 绑定。之所以选择 Rust 是因为它的内存安全性、对我们计划构建集成的多种编程语言的绑定，以及它对 WebAssembly 的支持。语言封装器可用于以下语言：
...
该 SDK，以及基于它构建的 Secrets Manager CLI，可用于执行以下操作：
