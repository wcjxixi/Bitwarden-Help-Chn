...
* **设备登录**：从 Bitwarden 客户端发起身份验证请求，并通过在您已登录的设备上批准该请求来完成身份验证。[了解它的工作原理](../account/log-in-and-unlock/more-log-in-unlock-methods/log-in-with-device.md)。
* **通行密钥登录**：使用通行密钥登录 Bitwarden 客户端，如果该通行密钥支持 PRF，则使用它来解密您的密码库数据。[了解它的工作原理](../account/log-in-and-unlock/more-log-in-unlock-methods/log-in-with-passkeys.md)。
