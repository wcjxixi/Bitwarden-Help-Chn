...
在将任何内容发送到云服务器进行存储之前，Bitwarden **始终**会在本地设备上加密和/或哈希数据。**Bitwarden 服务器仅用于存储已被加密的数据**。更多信息，请参阅[加密](../encryption/encryption-protocols.md)。
...
不信任 Bitwarden 服务器吗？没关系。开源的力量让一切变得简单美好，您可以轻松地自己托管整个 Bitwarden 系统，您的数据由你自己控制。[了解更多](../../self-hosting/deploy-and-configure/docker/linux-standard-deployment.md)。
