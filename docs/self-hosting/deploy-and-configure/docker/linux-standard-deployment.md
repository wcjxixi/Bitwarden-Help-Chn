...
4、从 [**https://bitwarden.com/host**](https://bitwarden.com/host) 获取安装 ID 和安装密钥用于安装过程。更多信息，请参阅[我的安装 ID 和安装密钥是用来干什么的？](../../hosting-faqs.md#q-what-are-my-installation-id-and-installation-key-used-for)。
...
{% hint style="info" %}
Bitwarden 要求在整个部署过程中始终如一使用 HTTP 或 HTTPS。混合使用协议（例如，代理服务器使用 HTTPS，内部使用 HTTP）会导致连接、身份验证和同步错误。我们建议在生产环境中使用 HTTPS，仅在测试环境中使用 HTTP。
{% endhint %}
...
使用一系列 [Docker 容器](https://docs.docker.com/get-started/)在您的机器上部署和运行 Bitwarden。Bitwarden 可以使用任何 Docker 版本或计划运行。请评估哪个版本最适合您的安装。容器的部署是通过 [Docker Compose](https://docs.docker.com/compose/) 来协调的。当您下载 Docker Engine 时，Docker Compose 会作为插件自动安装。
...
为您的 Linux 服务器配置一个专用的 `bitwarden` 服务账户，用来安装和运行 Bitwarden。这样做可以将您的 Bitwarden 实例与服务器上运行的其他应用程序隔离开来。更多信息，请参阅 Docker 的[用于 Linux 的后续安装步骤](https://docs.docker.com/engine/install/linux-postinstall/)文档。
...
Bitwarden 提供了一个 Shell 脚本，可以在 Linux 和 macOS (Bash) 或 Windows (PowerShell) 上轻松完成安装。完成以下步骤以使用 Shell 脚本安装 Bitwarden：

1、将 Bitwarden 安装脚本 (`bitwarden.sh`) 下载到您机器上：

```shell
curl -Lso bitwarden.sh "https://func.bitwarden.com/api/dl/?app=self-host&platform=linux" && chmod 700 bitwarden.sh
```

2、运行安装脚本。将相对于 `bitwarden.sh` 的位置创建 `./bwdata` 目录：
...
*   **Do you want to generate a self-signed SSL certificate? (y/n)（您想生成自签名证书吗？）:**\
    （仅当选择 `n` 时才会显示 **Do you have a SSL certificate to use?**）指定 `y` 让 Bitwarden 为您生成自签名证书。这个选项只推荐用于测试。更多信息，请参阅[证书选项](../configuration-options/certificate-options.md)。
...
#### 环境变量（必需） <a href="#environment-variables-required" id="environment-variables-required"></a>

Bitwarden 的某些功能无法通过 `bitwarden.sh` 脚本进行配置。通过编辑位于 `./bwdata/env/global.override.env` 的环境文件来配置这些设置。**至少，您应该替换以下值**：
...
替换 `globalSettings__mail__smtp...=` 的占位符以连接到 SMTP 邮件服务器，该服务器将用于向新用户发送验证电子邮件以及向组织发送邀请。将电子邮箱地址添加到 `adminSettings__admins=` 将提供对管理门户的访问权限。
...
Bitwarden 安装脚本使用 `./bwdata/config.yml` 中的设置来生成a安装所需要的资产。某些安装场景（例如，在代理服务器后面使用备用端口进行安装）可能需要对 `config.yml` 进行调整，这些调整在标准安装过程中并未提供。

根据需要编辑 `config.yml` 文件，然后通过运行以下命令应用您的更改：
...
{% hint style="info" %}
首次启动 Bitwarden 时，可能会花费一些时间，因为它会从 Docker Container Registry 下载所有镜像。
{% endhint %}
...
<div align="left" data-with-frame="true"><figure><img src="https://bitwarden.com/assets/3Sq7MaJZ1jaEJUCW44wmwj/008be5ee5e43c20c8c840e71617e57eb/2025-05-05_15-34-44.png?w=1400&#x26;fm=avif" alt=""><figcaption><p>Docker 健康</p></figcaption></figure></div>
