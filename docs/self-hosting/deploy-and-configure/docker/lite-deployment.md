...
在运行 Bitwarden Lite 服务器之前，请先安装 Docker、设置 `settings.env` 文件，并决确定好数据库配置：

### 安装 Docker <a href="#install-docker" id="install-docker"></a>

Bitwarden Lite 使用 [Docker 容器](https://docs.docker.com/get-started/)运行在您的机器上。Lite 可以与任何 Docker 版本或计划一起运行，但您**在继续安装之前必须在您的机器上安装了 Docker**。请参阅以下 Docker 文档获取帮助：
...
与标准 Bitwarden 部署不同，Lite 并不附带现成的数据库。您可以使用现有数据库，也可以创建新数据库。您需要在 `settings.env` 文件或 `--env` 标志中包含哪些 `# Required Settings #` 将取决于您使用的受支持的数据库提供程序：
...
| --volume , -v  | 绑定挂载卷。至少需要挂载 `/etc/bitwarden`。                                                                                                                                                                                       |
| --publish , -p | 将容器端口映射到主机。该示例显示映射了 `80:8080` 端口。配置 SSL 时需要使用 `443` 端口。                                                                                                                                                              |
| --env-file     | [从其中读取环境变量的文件](lite-deployment.md#specify-environment-variables)的路径。或者，在同一行中使用 `--env` 标志来内联声明环境变量（[了解更多](https://docs.docker.com/engine/reference/commandline/run/#set-environment-variables--e---env---env-file)）。 |
...
| 变量                                         | 描述                                                                                                    |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| globalSettings\_\_mail\_\_replyToEmail     | 输入您的服务器的回复电子邮箱。                                                                                       |
| globalSettings\_\_mail\_\_smtp\_\_host     | 输入您的 SMTP 服务器的主机域名。                                                                                   |
