...
目前 Bitwarden 客户端和服务器端使用 `yyyy.mm.r` 约定，例如，`2022.5.0` 是 2022 年 (`2022.`) 年 5 月 (`5.`) 的基本版本 (`.0`)。如果后续发布修补程序，它们将变成 `2022.5.1`、`2022.5.2` 等。

最初的月度版本（以 `.0` 结尾的版本）由所有客户端和服务器端共用，（后续）当某些客户端和服务器端获得了修补程序（`.1`、`.2` 等），而其他还未获得时，版本号会有所偏差。
