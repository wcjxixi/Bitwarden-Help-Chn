...
{% hint style="info" %}
作为企业组织的成员，您需要遵守 SSO 策略，如果您无法使用**设备登录**选项，您需要改用 [SSO 登录](using-login-with-sso.md#login-using-sso)。
{% endhint %}
...
使用**设备登录**会将身份验证请求发送到您当前登录并已启用该选项的任何移动应用程序以获得批准。如果请求未获批准或拒绝，则请求将在 15 分钟后过期。如果您没有收到登录请求，请尝试从移动应用程序[手动同步您的密码库](../../password-manager/vault-administration/syncing-your-vault.md)。
...
1. 网页密码库客户端向 Bitwarden 数据库中的身份验证请求表发送一个请求，其中包括帐户电子邮件地址、唯一的身份验证请求公钥ᵃ 和访问代码。
2. 已注册的设备，即已登录并在 Bitwarden 数据库中存储了[特定于设备的 GUID](../../security/administrative-data.md) 的移动应用程序客户端，将收到此请求。
3. 请求获得批准后，移动应用程序客户端使用此请求中包含的身份验证请求公钥加密帐户的主密钥和主密码哈希。
...
ª - 身份验证请求公钥和私钥是为每一个无密码登录请求唯一生成的，并且只在请求存在时才存在。如果请求未被批准或拒绝，请求将在 15 分钟后过期并从数据库中清除。
