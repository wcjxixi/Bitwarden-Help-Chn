...
此命令将提示您输入**电子邮件地址**、**主密码**和**两步登录代码**（若[已启用](../../my-account/two-step-login/two-step-login-methods.md)）。CLI 目前支持[身份验证器](../../my-account/two-step-login/setup-guides/two-step-login-via-authenticator.md)、[电子邮件](../../my-account/two-step-login/setup-guides/two-step-login-via-email.md)或 [YubiKey](../../my-account/two-step-login/setup-guides/two-step-login-via-yubikey.md) 方式的两步登录。
...
{% hint style="success" %}
如果您的组织[要求 SSO](../../admin-console/organization-basics/enterprise-policies.md#single-sign-on-authentication)，您仍然可以使用 `--apikey` 登录 CLI。
{% endhint %}
...
如果您直接使用密码库数据，则[使用 API 密钥](bitwarden-cli.md#using-an-api-key)或 [SSO](bitwarden-cli.md#using-sso) 方式将要求您在 `login` 命令后紧接着使用显式 `bw unlock` 命令。
...
从 CLI 访问组织经常需要知道组织的 ID 以及单个[成员](../../admin-console/user-management/user-management.md)的 ID 和[集合](../../admin-console/organization-basics/collections.md)的 ID。
...
`move` 命令用于将密码库项目[转移到组织](../vault-basics/sharing.md)：
...
`confirm` 命令用于确认已接受邀请的[受邀成员](../../admin-console/user-management/user-management.md#confirm)加入您的组织：
...
{% hint style="info" %}
如果您的组织使用 [Key Connector](../../admin-console/login-with-sso/key-connector/about-key-connector.md) 并且您在[移除您的主密码](../../my-account/log-in-and-unlock/using-login-with-sso.md#login-using-sso)后使用 `--apikey` 选项登录，则需要运行 `bw config server --key-connector` 命令。
...
`import` 命令用于从之前的 Bitwarden 导出或[其他受支持的密码管理应用程序](../import-and-export/import-data-to-your-vault.md)中导入数据。该命令必须指向一个**文件**并包含如下参数：
...
* `--format encrypted_json`：导出为[加密的 .json](../import-and-export/encrypted-exports.md) 文件
...
用于指定在[登录](bitwarden-cli.md#log-in)时使用哪一种[两步登录方式](../../my-account/two-step-login/two-step-login-methods.md)：
...
与 `create` 命令一起使用以指定[项目的类型](../vault-basics/vault-items.md)：
...
与 `create` 和 `edit` 命令一起使用以配置[自定义字段](../vault-basics/custom-fields.md)：
...
指示[用户的类型](../../admin-console/user-management/user-types-and-access-control.md)：
...
指示[用户在组织内的状态](../../admin-console/user-management/user-management.md)：
