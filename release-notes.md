...
* **集合管理设置更新**：限制所有者和管理员创建和删除集合的设置已被分离为两个单独的设置，分别用于每种操作。了解更多有关集合管理的信息。
* **删除集合项目需要「可以管理」权限**： 现在删除集合项目需要「可以管理」权限。具有「可以编辑」权限的用户不再具有该权限。了解更多有关成员权限的信息。
...
* **计费系统迁移**：从本月开始，现有的提供商将开始迁移到更新的客户组织计费系统。了解更多。
...
* **浏览器扩展 - 支持预配置环境 URL**：自托管客户现在可以为浏览器扩展预配置环境 URL，通过使用端点管理来部署您的配置，从而简化最终用户的部署（参阅[这里](self-hosting/configure-clients-for-your-server.md#liu-lan-qi-kuo-zhan)）。
