...
* 新的设备登录保护：为确保您的账户安全，Bitwarden 将逐步开始要求未使用两步登录或 SSO 的用户进行额外验证。了解更多。
* 更新恢复密码的使用：使用恢复密码虽然仍需要您的电子邮件地址和主密码，但现在将自动登录您的保险库并停用两步登录，而不是仅停用两步登录。了解更多。
* 适用于 macOS 桌面的 FIDO2 两步登录：macOS 桌面应用程序现在支持使用 FIDO2 密钥的两步登录。了解更多。

### Password Manager

* 点击自动填写设置已移动：浏览器扩展上的 "点击自动填写 "设置已移至 "设置"→"外观 "选项卡。了解更多。
* 防止 iOS 上的重复密码：在 iOS 上无法保存与 Bitwarden 保管库中已存储的现有用户名和服务相匹配的重复密钥。现有密钥可能会被修改或覆盖。了解更多有关创建密钥的信息。
* 企业单点登录更新：在单点登录工作流程的第一步添加了 "使用单点登录 "按钮，以简化企业单点登录。了解更多。

### 管理控制台 <a href="#admin-console" id="admin-console"></a>

* 移除使用 PIN 码解锁策略：企业组织现在可以设置策略，禁止成员在客户端应用程序中使用密码解锁。了解更多。
* 政策不合规变更：以前因不合规而从组织中删除成员的策略，现在将改为撤销这些成员。了解更多。
* 设备批准请求的电子邮件通知：每当组织成员提交可信设备批准请求时，管理员现在都会收到一封电子邮件。了解更多。

### 提供商门户 <a href="#provider-portal" id="provider-portal"></a>

* 将现有机构添加到 "提供方门户"：如果医疗服务提供者用户也是机构的所有者，他们现在可以将现有机构添加到 "医疗服务提供者门户"。了解更多。
