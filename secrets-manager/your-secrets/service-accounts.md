...
2、输入**服务账户名称**，然后在「访问权限」部分，输入或选择该服务账户应该能够访问的工程的名称。
...
在测试期间，所有成员都将在分配时获得对服务账户和相关机密的**读取和写入**访问权限。
...
2、从「工程」下拉列表中，键入或选择要添加到服务账户的工程。选择正确的工程后，选择**添加**按钮：
...
要删除服务账户，请使用要删除的服务账户的 (**≡**) 选项菜单选择**删除服务账户**。删除服务账户不会删除与其关联的机密。服务账户一旦删除就会被完全删除，**不会**像[机密那样被发送到回收站](secrets.md#delete-a-secret)。
