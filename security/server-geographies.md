...
要选择在哪个 Bitwarden 服务器地理位置上创建您的账户或组织，请选择登录或注册界面上的**区域**下拉菜单，然后选择您所需的区域，例如在 Web 应用程序中：
...
自托管 Bitwarden 组织或个人高级计划需要首先在云服务器上启动订阅，然后[上传许可证文件](../self-hosting/licensing-for-paid-features.md)到您的自托管实例。如果您在 EU 服务器上创建订阅，请将以下[环境变量](../self-hosting/configure-environment-variables.md)添加到服务器的 `./bwdata/env/global.override.env` 文件中，以确保您与正确的服务器通信：
