...
| globalSettings\_\_duo\_\_aKey=                        | 随机生成的 Duo 密钥。有关详细信息，请参阅 [Duo 文档](https://duo.com/docs/duoweb-v2#1.-generate-an-akey)。                                     | 用于[组织层面的 DUO 两步登录](../two-step-login/setup-guides/two-step-login-via-duo.md)。                                       |
...
创建云端组织后，请按照[这些说明](licensing-for-paid-features.md#organization-license)从云端获取您的许可证并将其上传到您的自托管服务器以创建该组织的自托管副本。

自托管的 Bitwarden 组织将能够使用他们选择的计划提供的所有付费功能。只有家庭和企业组织可以导入到自托管服务器。在[这里](../plans-and-pricing/about-bitwarden-plans.md)了解更多。
...
* 为企业组织的成员解锁[家庭组织赞助](../plans-and-pricing/redeem-families-sponsorship.md)。

按照[这些说明](licensing-for-paid-features.md#update-organization-license)为您的组织设置计费和许可同步。

{% hint style="info" %}
计费和许可证同步需要将 `globalSettings__enableCloudCommunication=` 环境变量设置为 `true`（[了解更多](configure-environment-variables.md)）。
{% endhint %}
...
您的企业需要独特的安全需求。使用策略为所有团队成员构建一致的部署和体验，例如要求 SSO 身份验证或在管理员密码重置中注册成员。为了让您的组织为更多团队成员做好准备，[尽早制定策略](../organizations/enterprise-policies.md)非常重要。
