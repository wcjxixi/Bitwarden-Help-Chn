...
在网页密码库的登录界面，输入您的电子邮件地址并选择**继续**。然后，选择**使用设备登录**选项：
...
1. 网页密码库客户端向 Bitwarden 数据库中的身份验证请求表 POST 一个请求，其中包括账户电子邮件地址、唯一的身份验证请求公钥ᵃ 和访问代码。
...
4. 批准客户端然后将已加密的主密钥和加密的主密码哈希 PUT 到身份验证请求记录，并将请求标记为已完成。
5. 发起客户端获取已加密的主密钥和已加密的主密码哈希。
6. 然后，起客户端使用身份验证请求私钥在**本地**解密主密钥和主密码哈希。
7. 然后，起客户端使用访问代码和已完成的身份验证请求通过 Bitwarden Identity 服务对用户进行身份验证。
